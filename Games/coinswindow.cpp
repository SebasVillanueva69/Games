...
void CoinsWindow::updateCountdown()
{
    ...
    if(actualTime.toString("m:ss") == "0:00")
    {
        ...
        if (playerCoinCount > 0)
        {
            ...
            congrats.setText("Has ganado, tu cuenta de monedas es: " + QString::number(playerCoinCount));
            congrats.exec();
            ui->goBack->hide();
            won = true;
        }
            ...
            QMessageBox sorry;
            sorry.setText("Has perdido, tu cuenta de monedas es: "+ QString::number(playerCoinCount));
            sorry.exec();
            ui->goBack->hide();
        }
    }
